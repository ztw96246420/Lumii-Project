...
- `mobile/src/mvp/LumiiMvpApp.tsx`
  ...
  - Screen26 电子宠物首页二次收敛：补宠物舞台 halo/ring、浮层聊天提示、在线徽标、健康分圆环和 2x2 快捷卡间距，使首页更接近 Figma Make 源码；2026-06-16 追加标题 `☀️` 与源码式纯文字聊天提示气泡；2026-06-18 追加将四宫格「健康备忘」入口收敛为「健康日历」。
  - Screen27/28 AI 对话页二次收敛：改为固定底部输入结构、横向快捷话题、头像在线点、安全提示和点状 typing 气泡，减少自写流程感；异常态补“连接中断”头部、灰色头像/状态点与红色错误 banner。
  - Screen29 健康首页二次收敛：改回 Figma Make 总览页结构，补顶部右侧新增入口、健康分渐变 Hero、体重趋势小图、三条入口卡和近期记录样式；2026-06-18 起右上角新增入口直接进入新增健康备忘，近期记录查看入口进入健康日历。
  ...
  - Screen33/92-96 健康备忘链路二次收敛：拆出独立新增备忘页，列表页补 Screen96 空状态，编辑页补日期/分类双行元信息卡、居中删除入口，并让保存成功停留在编辑页展示 Toast；新增页默认假内容已清空，需用户选择类型并填写备注；2026-06-18 起健康备忘列表页退出主流程入口，新增/编辑/删除后统一回到健康日历。
  ...
  - Screen75-80 健康日历首轮接入：新增 `healthCalendar` route，复用 `/health/calendar` 聚合数据，按 Figma 源码补宠物 mini card、月份切换、7 列日历、三色事件点、选中日期事件/空状态、逾期/临近提醒卡、加载骨架和读取失败页；2026-06-18 起宠物 mini card 删除健康分展示，避免与首页/健康首页重复。
