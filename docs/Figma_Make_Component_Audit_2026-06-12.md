...
- `mobile/src/mvp/LumiiMvpApp.tsx`
  ...
  - Screen21-25 AI 形象结果状态二次收敛：结果页补三候选卡和选中态，新增“不像我家宠物”反馈 Bottom Sheet、重新生成专用确认弹窗，并把保存成功/失败 Toast 改为 Figma 源码的深色成功胶囊与白底错误卡结构。2026-06-16 追加：当前真实 `aiResult` 路由按 Screen21 多候选态收敛，移除混入的 Screen20 原图 chip 和 hero badge，主形象改为 230px；Screen20 单结果默认态后续需要按真实单候选状态单独拆出。
  ...
  - Screen54/55 我的与宠物档案二次收敛：我的页菜单行支持 Figma 源码的不同图标底色、通知红色徽标和末行无分割线；本轮进一步修正 Screen54 标题 26px、菜单 28px 图标块、15px 常规字重标题、宠物性别/品种 badge、fallback 标签，并将我的页改为 route 级 edge-to-edge 容器，避免通用 Screen padding 在真机叠加；2026-06-16 根据真机反馈将我的页三块主内容横向留白从源码 16px 收紧为 12dp，降低大屏设备上的缩窄感；宠物档案详情改为源码同款全宽照片 Hero、基础信息右值列和健康区双入口。
