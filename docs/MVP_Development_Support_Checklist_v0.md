...
- ~~单手机号频控规则：每分钟、每天、每 IP 上限。~~ MVP 测试后端已支持单手机号 60s 冷却与每日上限（默认 `SMS_DAILY_LIMIT=50`）；生产仍需补 IP/设备级风控、真实随机验证码和短信服务商发送回执。
...
第一批最需要：
1. ~~短信验证码接口协议和测试手机号。~~ 测试后端已可用，并已支持单手机号 60s 冷却与每日上限；生产短信服务商、IP/设备级风控和真实随机验证码规则仍待确认。
2. ~~宠物资料接口协议。~~ 测试后端已可用。
