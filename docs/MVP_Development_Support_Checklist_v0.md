...
- ~~单手机号频控规则：每分钟、每天、每 IP 上限。~~ MVP 测试后端已支持单手机号 60s 冷却、单手机号每日上限、单设备每日上限和单 IP 每日上限（默认 `SMS_DAILY_LIMIT=50`、`SMS_DEVICE_DAILY_LIMIT=80`、`SMS_IP_DAILY_LIMIT=150`）；生产仍需补真实随机验证码、短信服务商发送回执、黑名单/WAF/验证码风控平台等更完整策略。
