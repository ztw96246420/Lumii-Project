...
- ~~点评提交、审核状态。~~ 已接测试后端；地点详情页的审核中回显按当前地点的我的点评记录驱动，点评输入和新增地点体验输入已隔离，不会被其他地点或新增地点审核状态串页。~~地点点评图片上传仍待设计/接口。~~ App 端已支持新增地点/点评页本地相册选择、最多 3 张预览、点击移除和提交照片数量摘要；生产级云端图片上传、审核展示和失败重试仍待媒体接口/设计补齐。
