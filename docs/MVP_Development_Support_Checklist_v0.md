...
- 是否支持语音输入、图片输入。
- AI 回复是否需要流式输出。
- ~~高风险医疗拦截策略。~~ MVP 已在服务端补确定性门禁：呼吸困难、抽搐、误食高风险物、严重外伤、大出血、持续呕吐腹泻、拒食拒水等会先返回固定安全回复且不触发 DeepSeek；同时会自动创建紧急健康观察备忘和通知中心就医提醒。生产级敏感内容安全、审核策略和医疗风险评估仍待补。
