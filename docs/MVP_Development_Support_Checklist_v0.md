...
- 灰产词库扩展、复杂内容审核和安全教育卡片暂时不作为靠前开发项；宠友圈动态/评论基础举报与用户拉黑已接入，完整黑名单管理页和生产级审核流仍按生产合规策略再补。

2026-06-12 本轮补充：
- ~~验证码页语音验证码入口。~~ 当前已改为明确 toast：发送中/倒计时期间提示等待，MVP 暂未开放语音验证码；不新增页面、不发起假请求。
- ~~今日小事页心情标签与 AI 帮写草稿。~~ 已按正式产品决策移除心情、交友/约遛标签和 AI 润色；发布只保留正文、最多 6 张图片、地点和公开范围，不再生成或展示虚假标签。
...
- ~~个人资料编辑、多宠管理、备忘编辑/删除、体重编辑/删除/异常状态仍需 Figma Make 设计。~~ App 已接 `ownerEdit`、`multiPet`、`memoEdit` 和体重编辑 Bottom Sheet，并接入对应前后端逻辑。
...
需要支持：
- ~~短信验证码接口。~~ 后端已提供 `POST /auth/sms/send`、`POST /auth/sms/verify`；非生产 mock 使用固定测试码 `962464`，生产通过服务端 Spug 代理发送随机一次性验证码。
...
- ~~建档后当前宠物维度数据刷新。~~ App 已在新建宠物成功后刷新健康摘要、体重、疫苗/驱虫、提醒开关、备忘和 AI 用量，避免进入上传/首页后沿用旧宠物数据。
...
电子宠物首页 -> 宠物档案 -> 宠物日历 -> 疫苗/驱虫计划/体重记录/新增备忘 -> 保存记录 -> 首页健康摘要更新。

需要支持：
- ~~宠物健康摘要接口。~~ 已接 `GET /health/summary`，首页、健康页、体重页和疫苗页顶部摘要已优先读取该接口。
- ~~自动疫苗/驱虫计划模板。~~ 已取消自动创建。新宠物的计划列表必须为空；后端和 mock 会清理旧版本自动生成且未经用户修改的模板，只有用户主动新增的计划才会保留。
...
- ~~备忘基础接口。~~ 已接 `GET/POST/PATCH/DELETE /health/memos`；备忘已补字段白名单、标题/内容必填和长度限制、`HEALTH_MEMO_INVALID` 错误码。就诊记录字段规范仍待后续确认。
- ~~日历记录重复提交保护。~~ 体重、备忘和今日小事已有本地保存锁；健康首页快捷备忘保存成功后会清空标题和内容，新增备忘页也不再预填“驱虫/外用滴剂”等假内容，减少误用旧草稿。
...
- ~~日历日期和今日状态展示。~~ 健康首页、备忘列表/编辑页、体重历史、删除确认、疫苗/驱虫计划页和宠物日历提醒卡已统一用“今天 / 昨天 / X 月 X 日”展示真实日期，紧凑日期位使用“今天 / 明天 / 6.23”；不再把只有日期的数据伪造成当前时分。首页“今日体重”只有当天存在体重记录才显示“已记录”。
- ~~疫苗/驱虫提醒通知。~~ MVP 已支持提醒开关、通知中心提醒、真机本地系统通知调度；标记完成、关闭通知或退出账号会取消对应本地提醒；测试后端和 mock API 会在关闭提醒、标记完成或读取通知时清理旧提醒通知。
...
需要支持：
- ~~基于位置的附近用户列表。~~ 当前后端按运营后台 `social.discoverRadiusKm` 的 `3km / 5km / 10km` 档位返回附近用户、附近小事和地图 POI，首发默认 `10km`；客户端不能扩大服务端范围。
...
- ~~正式举报、拉黑、安全策略。~~ 宠友圈小事/评论、私信、地点/点评和招呼已接专用举报、拉黑/解除、证据快照、天御机审、人工复核、处置、申诉和后台审计闭环。
...
- ~~生产无 POI 时回退示例地点。~~ 生产环境不加载 seed 地点；高德失败时只使用半径内已保存的真实坐标地点或返回空列表。新增地点必须携带新鲜定位，审核后固定在提交坐标，不会随用户跨城移动。
...
- ~~黑名单接口。~~ 已接集中黑名单、拉黑、解除、双方社交可见性和后台关系排查。
- ~~账号注销流程。~~ 已接短信确认、30 天冷静期、全端会话撤销、冷静期恢复、到期业务数据清理、COS 对象销毁重试和匿名墓碑。
...
- ~~`POST /auth/token/refresh`：刷新 token。~~ 已支持签名 token 刷新、设备标识继承、刷新链撤销和 401 恢复；生产强制拒绝旧 `lumii-local-手机号` 明文格式，不再作兼容登录态。
- ~~`GET /me`：当前用户资料。~~ MVP 测试后端已支持，App 会用于“我的页”等用户资料同步。
- ~~`PATCH /me`：更新用户资料。~~ MVP 测试后端已支持更新 `ownerName`；完整资料编辑页仍需后续设计。
- ~~`POST /account/delete/request`：发起账号注销。~~ 已接生产短信发码、频控和不暴露验证码的响应。
- ~~`POST /account/delete/confirm`：短信确认注销。~~ 已接冷静期、全会话失效、恢复和到期永久清理。
...
- ~~单手机号频控规则：每分钟、每天、每 IP 上限。~~ 后端已支持单手机号 60s 冷却、手机号/设备/IP 日限额、验证码输错次数上限和生产随机验证码（默认 `SMS_DAILY_LIMIT=50`、`SMS_DEVICE_DAILY_LIMIT=80`、`SMS_IP_DAILY_LIMIT=150`、`SMS_VERIFY_MAX_ATTEMPTS=5`）；后续增强为黑名单/WAF、验证码风控平台和最终送达告警。
...
- ~~今日小事图片长期保存。~~ App 已将最多 6 张图片以 `pet_circle_photo` 上传并绑定发布时宠物，后端只接受当前账号、当前宠物、审核通过的媒体 URL；图片可跨设备读取，删除小事会同步解除业务引用并按对象清理策略回收。
...
- AI 对话已支持显式备忘写入：用户说“帮我记一下/记录一下/记到备忘”等意图时，后端会写入当前宠物备忘，回复携带 `createdMemo`，App 会刷新健康摘要和备忘列表；重复同一句会去重。
...
需要接口：
- ~~`GET /pets/{petId}/health/summary`：首页健康摘要。~~ 测试后端已接 `GET /health/summary`，按当前宠物聚合健康分、最近体重、疫苗/驱虫计划、备忘和提醒开关；首页/健康页已有基础 UI，不需要新增设计页面。
- ~~`GET /pets/{petId}/health/calendar`：宠物日历。~~ 测试后端已接 `GET /health/calendar`，按当前宠物聚合体重、疫苗/驱虫计划和备忘；宠物日历独立 UI 已接入。宠物首页、健康页近期记录和宠物详情健康区的主入口统一为宠物日历，备忘列表页不再作为主流程入口。
- ~~`POST /pets/{petId}/health/memos`：新增备忘。~~ 测试后端已接 `POST /health/memos`，按当前宠物持久化；当前入口从宠物日历空状态/健康页加号进入，保存后回到宠物日历。
- ~~`PATCH /pets/{petId}/health/memos/{memoId}`：编辑备忘。~~ 测试后端已接 `PATCH /health/memos/{memoId}`，按当前宠物持久化；字段白名单、标题/内容长度、错误码和 RN 编辑页已补，当前主要从宠物日历事件进入编辑。
- ~~`DELETE /pets/{petId}/health/memos/{memoId}`：删除备忘。~~ 测试后端已接 `DELETE /health/memos/{memoId}`，按当前宠物持久化；RN 已补删除二次确认，删除后回到宠物日历。
...
- ~~`GET /pets/{petId}/vaccines/plan`：疫苗/驱虫计划。~~ 测试后端已接 `GET /health/vaccines`。
- ~~新增疫苗/驱虫计划。~~ 测试后端、mock API 和 App 已接 `POST /health/vaccines`；现有疫苗/驱虫计划页内联新增表单支持名称、年月日选择、保存 loading 和失败 toast，不新增页面。
- ~~`PATCH /pets/{petId}/vaccines/plan`：更新疫苗/驱虫计划。~~ 测试后端已接 `PATCH /health/vaccines/{vaccineId}`，支持标记完成；只接受 `due/done/overdue` 状态，非法字段或非法状态返回 `HEALTH_VACCINE_INVALID`；`due` 在前端统一显示“计划中”。
...
- ~~体重记录、备忘、今日小事和健康摘要当前宠物状态保护。~~ App 已补账号 token 与当前宠物 ID 校验，切号或切宠后的旧回包不会写入当前宠物健康数据，也不会清空新宠物输入草稿；不需要你额外补页面或接口。
...
- ~~拉黑/解除拉黑。~~ 已以 `/social/owners/{ownerId}/block`、`/social/blocks`、`/social/blocks/{ownerId}` 与相关对象级入口接入。
- ~~正式举报。~~ 已以小事、评论、私信、地点点评和地点本体的对象级接口接入统一 `socialReports` 审核闭环。

需要你确认：
- ~~附近范围：1km、3km、5km、10km 是否可选。~~ 已收口为后台 `3km / 5km / 10km` 三档，首发默认 10km；服务端强制执行，客户端参数不能扩大范围。
...
- ~~是否接入地图 POI，还是先做自有地点库。~~ 已采用 POI 打底 + 自有宠物友好层；生产必须配置高德 Web 服务 Key，缺失时上线台账形成 P0 阻断，不会伪造示例结果。
...
- ~~疫苗/驱虫提醒与系统通知状态提示。~~ 提醒开启后，如果系统通知权限或 App 通知开关未开启，App 会明确提示“系统通知需在设置中开启”；权限和开关都开启时会安排真机本地系统通知，并在标记完成、关闭通知或退出账号时取消；通知中心会自动去重并清理已关闭或已完成计划的旧提醒。
...
- ~~`GET /social/blocks`：集中黑名单管理页读取列表。~~ MVP 测试后端、HTTP API 门面、mock API 和安全中心列表已接入。
- ~~`DELETE /social/blocks/{ownerId}`：集中黑名单管理页解除拉黑。~~ MVP 测试后端、HTTP API 门面、mock API 和安全中心解除按钮已接入；解除后双方重新按附近可见、定位距离和动态可见性规则展示。
- ~~`POST /feedback`。~~ MVP 测试后端和 mock API 已支持普通产品反馈提交；当前 App 已复用它承接招呼请求安全反馈，宠友圈动态/评论举报和拉黑改走专用接口。
...
第一批最需要：
1. ~~短信验证码接口协议和测试手机号。~~ 测试与生产后端均已可用，生产采用 Spug 服务端代理、随机一次性验证码、60s 冷却及手机号/设备/IP 日限额；后续继续增强 WAF、黑名单和送达告警。
2. ~~宠物资料接口协议。~~ 测试后端已可用。
...
5. ~~正式举报、拉黑、账号注销接口。~~ 已完成移动端、后端、运营后台与审计闭环。
...
- ~~MVP 登录态 token 方案。~~ 当前后端使用 HMAC 签名 token，默认 30 天有效期，支持刷新滚动签发、同设备刷新链撤销、用户端设备列表、退出指定设备、退出其他设备、后台会话审计和运行中 401 恢复；后续迁移独立认证服务时需保持同等设备撤销与审计能力。
