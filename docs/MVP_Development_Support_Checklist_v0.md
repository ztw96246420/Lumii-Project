...
- ~~健康日期和今日状态展示。~~ 健康首页、备忘列表/编辑页、体重历史和删除确认已统一用“今天 / 昨天 / X 月 X 日”展示真实日期；不再把只有日期的数据伪造成当前时分。首页“今日体重”只有当天存在体重记录才显示“已记录”。
...
- ~~地点搜索、分类筛选。~~ 当前 RN 地图页已支持测试后端搜索、分类筛选、loading 和空结果状态；搜索筛选面板里的距离最近/评分最高已接真实排序，1km/3km/5km 距离筛选会真实过滤地点；汪星/喵星友好已按地点 `supportedSpecies` 字段真实筛选，旧地点缺字段时仅按标签/类别保守推断；点评最多因当前地点接口没有点评数字段会明确提示待补字段，不做假排序。
