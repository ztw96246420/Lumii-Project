...
- ~~健康记录重复提交保护。~~ 体重、健康备忘和今日小事已有本地保存锁；健康首页快捷备忘保存成功后会清空标题和内容，减少误用旧草稿。
...
- ~~约遛草稿保存。~~ Screen39 已支持按当前账号手机号 + 对方 ownerId 本地保存约遛地点、时间、留言和保存时间；从发现页或聊天页再次进入同一伙伴约遛页会恢复草稿，发送成功后清理对应草稿。
