...
- 当前前端形态：React Native/Expo MVP 已使用 mock API 和状态机跑通主链路；页面不再长期依赖 Stitch iframe/HTML 文案识别，正在按 Figma Make 源码逐屏转译。
...
3. 高德地图：
   ...
   - 后续仍需要确认 POI 搜索/周边地点策略、外部导航跳转规则，以及 iOS 高德 Key/SDK。
4. QA 验收：
   - 设置页已增加“MVP 验收入口”，方便你下飞机后逐页验收所有页面/状态。
   - 2026-06-02 已完成首页、健康、体重、疫苗、发现、消息、通知、地图、地点详情、我的、宠物档案、设置、账号安全、安全中心等现有 RN 路由的 Figma Make 风格批量转译。
   - 该入口后续需要改成开发环境开关，正式包隐藏。
...
需要你确认：
- 验证码长度：当前按 6 位实现。
- 倒计时秒数：建议 60s，前端后续接真实倒计时。
- 单手机号频控规则：每分钟、每天、每 IP 上限。
- 测试手机号和固定验证码：测试手机号已验证可收短信；固定验证码仅用于命令行测试，真实前端会生成随机 6 位。
...
需要接口：
- `GET /social/discover`：附近宠物主人列表。
- `POST /social/greetings`：发送打招呼。
- `GET /social/greetings/inbox`：收到的招呼。
- `POST /social/greetings/{id}/accept`：接受招呼。
- `POST /social/greetings/{id}/reject`：拒绝招呼。
...
本轮已把前端入口从 Stitch iframe/文案点击桥接切换为 React Native 原生化 MVP 状态机。Stitch 仍作为视觉源和 QA 对照，但业务流程不再依赖页面全文识别或按钮文案推断。
