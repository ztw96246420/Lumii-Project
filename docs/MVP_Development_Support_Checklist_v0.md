...
- ~~假宠物资料和假照片占位清理。~~ 我的页宠物标签改为真实性格/健康资料推导，宠物档案体型改为基于真实体重粗分、毛色保持待识别；今日小事和地点点评未选照片时只显示空占位，不再展示示例图冒充用户图片；账号安全/我的页不再硬写实名等级、登录地域和设备数量；上传识别/生成/确认链路丢失真实媒体或任务时不再展示 demo 金毛图，改为引导重新上传。
