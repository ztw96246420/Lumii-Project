...
- 与用户当前位置的距离。
- 外部导航能力。
- 可选增强信息：电话、营业时间、商圈、评分、地点图片、导航入口坐标。

这些增强字段依赖 POI 类型和地图供应商覆盖情况，不保证每个地点都有。App 展示原则是：有值则展示；电话、营业时间缺失时显示待补充；图片缺失时回退为本地示意图。
...
当前地点主列表已接入 Lumii 后端高德 POI 聚合层。非生产 Web 预览可明确使用 seed 数据；生产环境不加载 seed，没有定位、没有 Web Service Key、外部接口失败或无有效 POI 时，只返回半径内已保存的真实坐标地点或空列表。
...
- ~~3km/5km 距离筛选主要依赖地点对象中的距离字段，不是后端基于经纬度实时计算。~~ 后端已统一按查询坐标与地点固定坐标计算 3km/5km/10km 范围和距离文案；无坐标存量地点不进入生产附近列表。
- 用户新增地点必须携带新鲜定位，提交后先进入审核中；审核通过时 manual 地点继承提交坐标，之后按该地点坐标进入附近列表，不跟随提交者当前位置。
- 高德 POI 只能作为“候选宠物友好地点”，仍需要 Lumii 自有点评、审核和运营数据确认。
- 2026-06-16 已补充 POI 增强字段抓取：后端请求 `show_fields=business,photos,navi`，并把电话、营业时间、商圈、图片和导航入口坐标标准化到 Lumii `Place`。
...
5. 提取 `business`、`photos`、`navi` 字段，补齐电话、营业时间、地点图片和入口坐标。
6. 与 Lumii 自有宠物友好层合并。
7. 返回给 App。
...
- `show_fields`：当前测试后端使用 `business,photos,navi`。
