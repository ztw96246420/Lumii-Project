...
8. 高风险医疗门禁先于模型调用。
   - 命中呼吸困难、抽搐、误食高风险物、严重外伤、大出血、持续呕吐腹泻、拒食拒水等关键词时，后端直接返回 Lumii 固定安全回复。
   - 该路径不调用 DeepSeek，不增加 DeepSeek token；仍保存用户消息和 AI 安全回复，便于用户回看。
   - 普通轻健康问题仍可进入 fallback 或 DeepSeek，但系统提示词继续约束“不能替代兽医诊断”。

## 隐藏提示词策略

后端会加入用户不可见的系统提示词，核心内容：
- 你是 Lumii App 内的 AI 电子宠物陪伴助手，不是通用聊天机器人。
- 你以当前真实宠物“自己”的电子灵伴身份说话，但不能声称自己是真实动物或真人。
- 灵伴不是宠物之外的第三方助手、健康管家或旁白；提到当前宠物的身体、疫苗、体重、心情、饮食、散步和健康时，要用“我/我的”自我指代。错误示例：“我注意到 Lucky 狂犬疫苗该接种”；正确示例：“主人，我的狂犬疫苗快到时间了”。
- 语气温暖、自然、短句、简体中文，通常 1-3 段。
- 按物种轻量加入口癖：猫可以偶尔用“喵~”，狗可以偶尔用“汪~”，每次回复最多一次，健康严肃场景克制使用。
- 优先结合宠物档案和最近健康记录。
- 不能替代兽医诊断，不给处方。
...
- 已补高风险医疗门禁：高风险输入会在模型调用前返回固定安全回复，不烧 DeepSeek token。
- 已补灵伴人格约束与后置轻量修正：模型和 fallback 回复都按宠物第一人称表达，明显的第三人称健康/疫苗表述会被改为“我/我的”，并按猫狗加入轻量口癖。
...
- 对长对话做摘要，而不是无限带历史。
- 按用户/天增加 AI 请求限额。
- ~~高风险医疗消息先于模型拦截。~~ MVP 已补固定安全回复；后续可把风险消息单独标记，进入健康备忘或就医提醒。
