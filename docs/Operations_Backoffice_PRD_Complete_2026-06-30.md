...
- 后台治理：`opsConfig`、`opsConfigRevisions`、`adminAuditLogs`
...
- 工作台概览。
- 用户列表、用户详情聚合、运营备注、风险标签、快捷禁言/冻结、用户业务数据清理。
- 宠物档案只读排查。
...
- 真实厂商 Push、推送回执和发送审批未接入；后台系统通知频控已基础接入。
- 数据看板已补页面、发现、地图、地点和通知等基础事件；严格留存、完整转化漏斗和 Push 厂商回执仍未做。
- 导出已支持基础筛选、原因必填和 CSV 水印；双人导出审批、异步大文件和下载过期机制未做。
...
| `pet.view` | 查看宠物档案 |
| `pet.edit` | 修正宠物资料 |
| `pet.media_moderate` | 隐藏或清理宠物头像、AI 形象、封面 |
...
已实现 `GET /admin/launch/readiness`，覆盖模块成熟度、上线前必须确认的问题、生产风险台账和配置联动关注项；页面口径区分“测试可用 / 部分可用 / 生产阻断 / 已预留”，避免把预留能力误标为生产完成。
...
已实现只读排查页 `GET /admin/pet-calendar`。
...
已实现只读排查页 `GET /admin/social-relations`。
...
- 用户重开。
- 移动端我的反馈。
- 工单 SLA 配置化，按 urgent/high/normal/low 配置小时数，并联动后台排序、统计、导出和移动端预计响应文案。
- 反馈收集页状态更新会同步到关联工单；正式回复用户仍在工单中心完成。

待补齐：

- 负责人枚举。
- 客服排班、首响 SLA/解决 SLA 拆分。
- 客服质量统计。
- 批量回复审批。
...
- 送达数量。
- 跳过数量。
- 状态：draft、scheduled、sent、failed、canceled。
- 发送人。
- 预约时间。
- actionRoute。
- 是否尊重用户开关。
...
- 回滚生成新的版本，而不是删除历史。
- 移动端公开配置不能返回敏感关键词明文。
- 高风险配置需要二次确认：维护模式、强制更新、全功能关闭、内容安全规则。
- 生产期配置发布应支持草稿、审批、定时发布和灰度。
...
- 严格留存 Cohort 不完整，仍需要独立事件表、设备去重和长期窗口。
- Push 真实送达/点击缺厂商回执。
...
- 工单 SLA 配置已基础落地；生产期补负责人枚举、客服排班和首响/解决 SLA 拆分。
- 真实厂商 Push 基础通道。
- 数据导出双人审批和敏感字段授权。
...
12. 系统通知是否需要发送审批和灰度人群包？频控已按配置中心接入。
13. 配置强制更新是否必须审批？
