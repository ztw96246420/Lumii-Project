...
- 导出已支持基础筛选、原因必填、CSV 水印、默认敏感字段脱敏、完整敏感字段独立授权、审批流、高风险最少会签人数、审批有效期、单审批下载次数上限和服务器本地归档任务；对象存储归档和签名下载链接未做。
- 操作审计仍保存在 JSON state 内，但新审计记录已写入 `prevHash/hash` 哈希链，可在后台校验 retained window 内的准不可篡改完整性；生产期仍需同步到数据库、WORM 或外部日志服务。
...
后台审计页已展示匹配日志、高风险动作、缺少原因、链路状态、已签名日志和最新 Hash 等指标；从 2026-06-30 起，后台登录和后台请求触发的审计会记录 IP 与 User-Agent，历史审计可能为空。从当前版本开始，新审计记录会写入 `prevHash/hash` 形成哈希链，旧日志会标记为历史未签名，不伪装成已验证。
...
- 生产期仍需把审计 hash 链同步到数据库、WORM 存储或外部日志服务，避免只依赖单文件 JSON state。
...
- 真实厂商 Push 基础通道。
- 数据导出已接审批流、高风险最少会签人数、默认敏感字段脱敏、完整敏感字段独立授权和服务器本地归档任务；生产期补对象存储归档、签名下载链接和文件生命周期策略。
