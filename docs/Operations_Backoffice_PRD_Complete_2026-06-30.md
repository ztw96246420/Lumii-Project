...
- 图片内容审核、头像审核、宠物图审核、地点图片审核还未形成统一流程。
- 用户备注和风险标签已有基础能力；生产期仍需角色权限、系统自动标签和备注作废流程。
- 宠物资料、头像、AI 形象、宠友圈封面的后台修复能力尚未开放。
...
- AI 形象状态不能只凭 `avatarUrl` 判断，应关联 ready 且已接受的任务。
...
已实现 `GET /admin/audit-logs`，记录多类后台写操作。
