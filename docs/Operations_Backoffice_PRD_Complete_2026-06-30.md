...
- 后台治理：`opsConfig`、`opsConfigDrafts`、`opsConfigApprovals`、`opsConfigSchedules`、`opsConfigRevisions`、`adminAuditLogs`

### 2.3 当前已落地的后台能力

当前测试后台已落地：

- 管理员登录、后台静态页托管、后台 API 鉴权。
- 工作台概览。
- 用户列表、用户详情聚合、用户时间线、运营备注、风险标签、快捷禁言/冻结、用户业务数据清理。
- 宠物档案只读排查。
- AI 灵伴任务、素材、反馈、供应商用量、重试、刷新、标记失败、返还额度。
...
- 没有多管理员账号、细角色、权限点、MFA、IP 白名单和登录风控。
...
- 用户备注和风险标签已有基础能力；生产期仍需角色权限、系统自动标签和备注作废流程。
- 宠物资料编辑、违规媒体清空和普通头像/宠友圈封面替换已支持；重复宠物合并尚未开放。
- 任意消息正文查看仍需更细权限/审批；私信单条消息举报已接入后台任务池、上下文审计查看、疑似骚扰标记和双方隐藏/删除闭环。
- 地点质量分、重复候选、地点详情编辑、地点图片审核、人工确认后的地点合并、自定义审核模板和基础地点贡献账本已接入；用户端公开贡献身份、等级或活动奖励仍待定。
...
当前代码已基础实现系统健康页、单 `admin` 账号权限现状页和上线台账页；多后台账号、细角色运行时拦截、操作审批、MFA/IP 白名单、内容安全模型和生产数据底座仍是生产级后台需要补齐的治理能力。
...
- `GET /admin/accounts`
- 单一 `admin` 账号。
- 账号权限页已展示当前 admin、会话、权限点、角色预留、安全检查、最近登录和最近高风险动作。
- 登录失败锁定已实现：连续 5 次密码错误默认锁定 15 分钟，失败、锁定、锁定期拦截和成功清零都会写审计。
...
已开放普通头像和宠友圈封面替换；暂未开放重复宠物合并和后台替换 AI 灵伴任务结果图。
...
- 所有修正必须填写原因，写入 `calendar.record.update` 审计，并通知用户。
- 修正的是移动端真实 `/health/*` 数据，不维护后台独立副本。
...
- AI 样本池：支持将任务沉淀为提示词优化样本、供应商异常样本或素材质量样本，支持待复盘、已复核、已忽略状态和 CSV 导出。
- 刷新、重试、标记失败、返还额度。
...
- 从已有关系行查看最近私信上下文窗口，必须填写查看原因并写 `social.relation.message_context.view` 审计。
...
- 基础地点贡献账本：发现新地点 `+10`，补充已有地点 `+5`，后台展示贡献记录和贡献分。

待补齐：

- 用户端公开贡献身份、贡献等级、活动奖励或兑换策略。
...
11. 地点贡献分是否对用户公开展示，是否接贡献等级、活动奖励或兑换规则？
