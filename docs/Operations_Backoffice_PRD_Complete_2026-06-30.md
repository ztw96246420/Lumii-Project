...
- 通知：`notifications`、`pushDevices`、`pushRegistrationDiagnostics`、`systemNotifications`
...
- 多管理员账号底座、角色权限运行时拦截、IP 白名单、逐账号登录失败锁定、TOTP MFA、密码轮换状态检查、离职凭据销毁/会话失效和生产安全配置包已接入；生产期仍需真实启用 MFA/IP 白名单/环境变量密码和正式账号数据库。
...
- 地点质量分、重复候选、地点详情编辑、地点图片审核、人工确认后的地点合并、自定义审核模板、基础地点贡献账本、贡献分手动调整和撤销、公开点评展示策略已接入；用户本人公开贡献身份、轻量等级、我的排名、匿名排行榜和荣誉候选策略已接入，真实兑换/活动结算仍待业务确认。
- Expo Push ticket/receipt、App 内通知展示/点击回传、后台系统通知频控、对象深链、发送审批、高风险最少会签人数和后台待审批值守队列已接入；FCM/Expo/后端登记分阶段诊断和失败设备追踪已接入。真实 Android 厂商 Push/APNs 专项通道和 OS 级展示回执未接入。
- 数据看板已补页面、发现、地图、地点、通知展示和通知点击等基础事件；严格留存、完整转化漏斗和 Push 厂商级回执仍未做。
- 配置中心已支持草稿、高风险确认、预约发布、版本回滚、发布审批、高风险最少会签人数和后台待审批值守队列；生产期仍需站外审批提醒、灰度配置和 A/B 分流。
- 导出已支持基础筛选、原因必填、CSV 水印、默认敏感字段脱敏、完整敏感字段独立授权、审批流、高风险最少会签人数、审批有效期、单审批下载次数上限、服务器本地归档任务、可选 COS 对象归档和短时效签名下载链接；生产期需配置 COS 生命周期、对象销毁审计和站外审批提醒。
...
当前代码已基础实现系统健康页、账号权限页、state 多管理员账号底座、角色权限运行时拦截、逐账号登录失败锁定、IP 白名单、TOTP MFA、密码轮换状态检查、离职停用、生产安全配置包、高风险最少会签人数、后台待审批值守队列、站外 Webhook 告警、脱敏 JSON 访问日志、`X-Request-Id` 请求追踪、上线台账页和合规文本签署页；内容安全质检策略复盘、真实生产安全配置、外部 CLS/APM 和正式值班闭环仍是生产级后台需要补齐的治理能力。
...
- `GET /admin/accounts`
- `POST /admin/accounts`
- `POST /admin/accounts/security-package`
...
- 生产安全配置包已实现：可生成一次性强密码、TOTP Secret、当前 IP 白名单建议、systemd override 和重启命令；生成动作写审计但不记录密码或 MFA Secret。

待补齐：

- 生产环境真实启用环境变量密码、全员 MFA 和 IP 白名单。
- 生产级历史密码策略。
- 登录设备管理。
- 管理员离职禁用。
...
已实现 `GET /admin/system/health`，覆盖后端运行、状态文件、状态备份、内存、关键外部服务配置、业务积压、JSON state 集合行数，以及结构化访问日志的启用状态、4xx/5xx/慢请求计数和最近错误时间。页面只展示配置是否存在，不展示密钥值；状态文件已接入原子写入、滚动 gzip 备份和启动损坏恢复，请求链路已接脱敏 JSON 日志与 `X-Request-Id`，生产期仍需把 journald 接入腾讯云 CLS/APM 并配置日志告警。

已实现 `GET /admin/launch/readiness`，覆盖模块成熟度、上线前必须确认的问题、生产风险台账和配置联动关注项；页面口径区分“测试可用 / 部分可用 / 生产阻断 / 已预留”，避免把预留能力误标为生产完成。已实现 `POST /admin/launch/readiness/questions/{questionId}`，运营可在页面内记录待澄清问题的状态、负责人/决策来源和备注，状态支持 `open`、`reviewing`、`ready`、`deferred`、`closed`，更新/重置均写入审计。已实现 `POST /admin/launch/readiness/signoff`，支持签署当前上线结论、保存 P0/待确认问题/模块状态快照、重置签署记录，并在存在未关闭 P0 时拒绝签署 `ready_for_production`。已实现合规文本签署联动：用户协议、隐私政策、内容审核制度和 App 备案材料全部签署后，`q-compliance-text` 自动从待签署收敛为已签署。
...
- 数据导出已接入 `pet_chat_messages`，可按手机号、关键词、时间筛选医疗风险、自动写入、机审拦截和运营复核样本；默认脱敏，完整敏感字段仍走敏感导出审批。
- AI 回复已保存轻量生成快照：provider、生成来源、模型参数、System Prompt hash/摘要、动态上下文 hash、宠物档案快照和 token 用量；移动端接口不会返回该后台追溯信息。
...
- 添加质量标签。
- 隐藏回复。
- 取消隐藏：必须填写原因并写审计；内容安全机审拦截的回复必须先复核为“样本正常”才能恢复展示。
- 加入医疗风险样本。
- 加入提示词优化样本。
...
- 医疗风险样本能在 AI 对话列表检索，并能通过 `pet_chat_messages` 导出；默认 CSV 脱敏，完整敏感字段需 `data.export.sensitive` 权限和单独审批。
...
- 基础地点贡献账本：发现新地点 `+10`，补充已有地点 `+5`，后台展示贡献记录、贡献分、贡献排行榜和荣誉候选策略，并支持运营手动调整或撤销贡献记录。

待补齐：

- 用户本人公开贡献身份、轻量等级、我的排名、匿名排行榜和荣誉候选策略已接入；后台纠偏会实时影响移动端汇总。真实活动结算、他人主页贡献身份或兑换规则仍待业务确认。
...
- 批量处罚审批：同一批目标账号提交后进入高风险审批，审批通过才逐个写入真实处罚流水并影响移动端写接口。

待补齐：

- 多管理员复核、站外审批提醒和生产值守 SOP。
- 处罚自动升级策略。
...
- 推送登记诊断：即使安装实例尚未取得 token，也展示权限/FCM/Expo/后端阶段、白名单失败码、App 版本、尝试次数和更新时间；完整 token 与原生错误原文不得进入后台列表。

待补齐：

- Android 厂商 Push/APNs 专项通道。
- OS 级展示回执和厂商级推送质量统计。
- Expo Push ticket/receipt、App 内通知展示/点击、移动端登记退避重试、失败阶段追踪、高风险最少会签人数和后台待审批值守队列已接入；生产期仍需提供真实 Firebase Android 配置与 EAS FCM V1 服务账号，完成真机 receipt 验收，并按需要评估厂商专项通道和 OS 级展示回执。
...
- 生产期配置发布仍应继续补齐站外审批提醒、审批后预约通知和灰度人群。
...
- 移动端根节点已接错误恢复页；渲染错误、全局 JS 错误和 Web 未处理 Promise 拒绝按账号本地作用域排队并在登录态恢复后补报 `app.runtime_error`。客户端和后端双重禁止异常原文、堆栈、手机号、Token 和用户输入落库。
- 系统健康按近 24 小时聚合移动端异常，运营告警按发生次数与影响账号升级；客户端自报只作为观测证据，不会单独形成可信 `bad` 定级。
- 生产级严格留存 Cohort 仍需要独立事件表、设备去重和长期窗口；当前为 JSON state 测试期轻量口径。
- Push 已接 Expo ticket/receipt 和 App 内展示/点击，仍缺 Android 厂商 Push/APNs 专项通道和 OS 级展示回执。
...
- 开启 `LUMII_ADMIN_EXPORT_COS_ENABLED=true` 且 COS 环境变量完整时，归档任务会同步写入 COS，后台展示对象归档状态、objectKey、归档时间和失败原因；COS 失败不影响本地归档下载，但会写入对象归档失败审计。
- 不导出图片二进制。
- 不导出完整设备 token。
...
- `PATCH /admin/legal-documents/{documentKey}`
- `POST /admin/legal-documents/{documentKey}/approve`
- `POST /admin/legal-documents/{documentKey}/reset`
- `GET /legal/terms`
- `GET /legal/privacy`
...
- 多管理员账号底座和生产安全配置包已接入；上线前继续补真实生产启用和账号交接治理。
- 角色权限运行时拦截。
- 内容安全文本/图片模型接入。
- 宠友圈图片审核。
- 头像/宠物图审核。
...
- 真实 Android 厂商 Push/APNs 专项通道。
- 数据导出已接审批流、高风险最少会签人数、默认敏感字段脱敏、完整敏感字段独立授权、服务器本地归档任务、可选 COS 对象归档和短时效签名下载链接；生产期补 COS 生命周期、销毁审计和归档失败告警。
...
11. 地点贡献分已支持用户本人公开徽章、轻量等级、我的排名、匿名排行榜、后台手动调整/撤销和荣誉候选策略；是否接真实活动结算、他人主页贡献身份或兑换规则？
