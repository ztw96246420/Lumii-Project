...
举报地点本体信息，例如名称、地址、电话、营业时间、宠物友好标签或地点照片不准确。成功后返回 `{ reported: true, targetType: 'place' }`。

后台联动：
- 创建 `socialReports[]` 举报记录，`targetType=place`。
- 生成举报证据快照，包含地点名称、地址、标签、电话、营业时间和公开地点图片引用。
- 进入统一内容安全任务池；后台可从该举报直接修正地点资料并把举报标记有效。
- 地点本体举报不直接在举报动作中下架地点，避免误伤高德或社区共建地点目录。
