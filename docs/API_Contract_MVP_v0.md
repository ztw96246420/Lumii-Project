...
- `expiresAt` 用于验证码过期判断。
- 2026-06-12 补充：MVP 测试后端已支持单手机号 60s 冷却和每日发送上限；默认 `SMS_DAILY_LIMIT=50`，触发每日上限时返回中文错误，前端复用现有 toast，不需要新增页面。
...
- ~~短信频控规则：单手机号、单 IP、单设备、每日上限。~~ 当前测试后端已做单手机号 60s 冷却与每日上限（默认 `SMS_DAILY_LIMIT=50`）；生产仍需补 IP/设备级风控、真实随机验证码与短信服务商回执。
