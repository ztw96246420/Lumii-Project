...
- 验证码页语音验证码入口不再是静态假链接：点击后会根据当前发送中/倒计时状态给出明确 toast；MVP 暂未开放语音验证码，不新增页面、不触发假发送。
- 今日小事页补现有控件联动：心情标签现在可选中；“AI 帮我写”会基于当前宠物名、选中心情、体重/疫苗摘要生成本地草稿，不调用新模型、不烧 token；发布仍沿用健康备忘保存链路。
- 新增地点页补现有特色标签联动：可遛狗、饮水点、室内友好、停车方便现在可选中，并会合并进提交审核内容；仅选择标签也可以形成有效体验描述，避免装饰标签无业务结果。
- 我的页入口语义收口：当前宠物区右上角不再显示会误导的“多宠管理”，改为“查看档案”；通知菜单项从“通知设置”改为“通知中心”，右侧显示真实未读数；个人资料编辑铅笔不再是无响应图标，点击会明确提示需补 Figma 设计后开放。
- 缺失设计清单补充：已新增“个人资料编辑页”Figma Make 提示词；多宠管理仍按原缺失页面推进，不在现有 App 内假跳转。
- 退出/切号清理补齐：每日心情和地点特色草稿会随本机账号状态一起清理，避免新账号看到旧账号临时选择。
- 验证：`cd mobile && npm run typecheck` 通过。根目录无 `package.json`，根目录执行 `npm run typecheck` 会失败，这是路径问题，不是代码问题。
...
- 执行焦点再次校准：后续每轮开发先检查现有页面里的业务断点，例如切号串号、权限/定位旧回包、按钮重复提交、页面回退后状态错乱、两台手机联动刷新；灰产词库扩展只保留现有基础拦截，不继续占用 MVP 主线。
- 后台动作旧账号回包保护补强：AI 用量读取、权限状态刷新、权限请求、权限快照保存、推送设备登记和地图定位都会校验当前账号 token；地图定位增加 ref 级互斥，切号或退出后的旧回包不会覆盖当前账号权限、AI 用量、push token 或地图中心。不新增页面，不接新接口。
- 发现页与设置联动当前性补强：发现页自动刷新、下拉刷新、定位授权和附近列表回包增加账号 token 与 request 序号校验；手动刷新时自动轮询会避让；关闭附近可见、切换模糊定位、退出账号或切号会立即废弃旧请求。启动全量加载会按账号设置里的 `nearbyVisible` 决定是否应用附近列表，避免附近可见已关闭时列表一闪而过；设置开关保存也补账号 token 校验。不新增页面，不接新接口。
- 登录恢复当前性补强：`restoreAfterLogin` 现在会绑定本次登录 token，连续登录、切号或退出后的旧宠物/权限回包不会改当前首屏路由；本地持久化 session 会写入刷新后的当前宠物、权限状态、权限引导完成状态和设置快照，二次打开更稳定。不新增页面，不接新接口。
...
- 文档口径二次同步：手机号/设备/IP 频控、生产 Spug 服务端代理和随机验证码已完成；当前剩余是最终送达告警、黑名单/WAF/验证码风控平台等增强。
...
- 全量加载/消息/通知旧回包保护补强：启动后的 `loadCommonData` 显式绑定当前登录 token；消息列表刷新、会话记录加载、通知批量已读和会话已读回包也会校验账号 token，切号或退出后的旧回包不会写入当前账号页面。不新增页面，不接新接口。
- 地点动作旧账号回包保护补强：地点收藏、地图搜索、地点点评和新增地点提交都会校验账号 token；新增地点还会确认仍在新增地点页后才清草稿和弹 toast，避免切号或退出后的旧回包污染收藏/地点结果或清空新账号输入。不新增页面，不接新接口。
