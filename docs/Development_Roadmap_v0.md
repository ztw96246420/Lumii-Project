...
当前实现状态：
- 移动端：React Native + Expo 已落地。
- Android：已接高德地图 SDK、权限、App 图标和真机安装包流程。
- 后端：~~MVP 推荐 Supabase + Edge Functions~~ 当前测试环境先使用腾讯云轻量服务器上的 Node.js 文件状态后端，便于快速联调；后续生产化仍需重新评估数据库、对象存储、鉴权和监控。
- 社交：附近发现、打招呼、招呼请求、接受/婉拒、会话、消息、未读归零已在测试后端跑通。
- AI：形象生成和 AI 对话仍以 mock 状态为主。
...
1. ~~评估高德地图和腾讯地图，确认 MVP 地图 SDK。~~ 已确认高德优先，Android SDK 已接入。
2. ~~确认短信服务商和手机号登录方案。~~ 非生产 mock 使用测试码 `962464`；生产由后端代理 Spug，使用随机一次性验证码，模板编号不进入 App。
3. ~~确认首批真实卡通化样张。~~ 已有金毛真实卡通化本地示例；仍需更多最终风格样张。
4. ~~画出移动端核心页面原型。~~ Stitch/Figma Make/Figma 源码包已形成首轮视觉源。
5. ~~搭建 React Native 项目。~~ 已完成。
6. ~~搭建数据库和认证。~~ 当前以腾讯云 Node 文件状态后端替代数据库做联调；生产数据库仍待选型。
7. ~~先做宠物档案和电子宠物生成闭环。~~ 宠物档案和 mock AI 生成闭环已跑通；真实 AI 生成接口仍待接入。
8. 下一步：补健康 CRUD、地点 POI/审核、举报拉黑、协议隐私、AI 真实服务。
