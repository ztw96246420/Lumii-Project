...
2026-07-09 已在多账号底座上补齐第一版运行时 RBAC：新增账号可选择 `super_admin`、`ops_admin`、`content_moderator`、`support`、`auditor` 等角色，后端会按权限点拦截高风险 API。这样先解决多人测试、离职禁用、密码重置、审计归因和基础最小权限问题，后续再叠加 MFA、逐账号风控和双人审批。
