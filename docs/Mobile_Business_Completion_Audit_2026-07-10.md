...
- 当前生产已启用 Expo Push 和 receipt 轮询，但真实设备 token 为 0、成功 receipt 为 0；代码链路与模拟供应商回归通过，仍需在 versionCode 13 真机授权通知后完成首台设备登记和真实送达验收。
...
- 开关采用兼容发布顺序：先分发支持同意参数的 versionCode 13，再启用强制校验；旧 versionCode 12 仍在用时不得提前开启。
...
- 全量可视上线门禁：`node scripts/smoke-launch-regression.cjs --include-visual` 于 2026-07-13 20:39 重新完整通过，79/79 套件全部成功；移动端 Playwright 覆盖协议阅读、本人宠友圈同日多条、审核中/驳回状态及昼夜排序差异、唯一“我”标记、评论、删除确认、地点贡献记录、地点点评驳回纠错、他人宠友圈权限、登录设备退出、运行中 Token 撤销恢复，以及疫苗/驱虫计划新增、编辑、提醒、完成、恢复和删除，后台 8 个关键运营页面同步通过。
...
- 生产台账实查：部署 `b26f6091` 后于 2026-07-13 返回 28 项健康检查、`bad=1`、`warn=3`、`openP0=6`、`blockedGaps=2`；唯一 `bad` 是兼容发布期有意暂缓的 `legal_consent_enforcement`，三项警告分别为首台真机 Push、后台 IP 白名单和站外告警。`public_api_https`、`public_api_external_https`、`backend_bind_address` 均为 `ok`。生产当前只有 1 个活跃管理员且未配置 MFA/IP 白名单，不能在没有真实审批人的情况下强开双人会签。
...
当前生产台账的 6 个 P0 由以下项目构成：后台安全综合项、协议同意强制开关、站外告警通道、IP 白名单决策、全员 MFA 决策、正式合规文本签署。后台安全综合项与 IP/MFA 两个决策存在口径重叠；协议强制开关需等待 versionCode 13 分发后开启，其余均为生产外部配置或业务签署，不代表新增页面或核心业务代码缺失。
