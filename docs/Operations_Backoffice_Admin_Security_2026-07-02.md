...
把当前单 `admin` 后台从“只有账号密码”推进到“生产可配置入口边界”的状态。

本次不引入多管理员或 MFA，避免超出当前单 admin 目标；先补齐生产前最基础的后端 IP 白名单能力，并让账号权限页、系统健康页和上线台账读取同一套状态。
...
- 后台安全整体仍会因为 MFA、多管理员和账号禁用/轮换未接而保持 `partial`，不误报为生产完全就绪。
