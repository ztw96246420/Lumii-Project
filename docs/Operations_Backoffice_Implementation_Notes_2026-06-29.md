...
- 用户可在工单未关闭前补充信息，补充后工单状态回到 `reviewing`，后台工单中心可继续处理。
...
10. 我的反馈第一版已支持进度详情和继续补充；后续是否需要附件、满意度评价和已关闭工单重新打开？
...
1. 处罚策略进阶：处罚模板、证据快照、多管理员复核、举报处理后自动生成处罚建议。
