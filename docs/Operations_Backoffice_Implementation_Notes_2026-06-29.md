...
- 新增地点审核通过会记录地点贡献者账本：发现新地点 `+10` 贡献分，补充已有地点 `+5` 贡献分；地点目录展示贡献者数量，后台新增贡献者卡片，并支持运营手动调整或撤销贡献记录。
...
- 贡献分当前已支持用户本人公开徽章、轻量等级展示和后台纠偏；排行榜、活动奖励或兑换规则仍需另行确认。
...
- 扩展回归脚本：`node scripts/smoke-place-contributions.cjs`，覆盖地点贡献账本、手动调整、撤销、`/me.placeContributionSummary`、贡献徽章配置发布和 `/app/config` 下发。
...
6. 新增地点审核通过/驳回已通知提交人，基础贡献积分账本、贡献身份展示和后台纠偏已落地。后续是否要公开排行榜、他人主页贡献身份或活动奖励待确认。
