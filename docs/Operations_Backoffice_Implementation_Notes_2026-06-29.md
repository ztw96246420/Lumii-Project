...
- AI 形象已应用通过 `avatarJobs.acceptedPetId` 与 `ready` 状态判断，避免仅凭 `avatarUrl` 误判。
- 表格展示主人、默认宠物、物种/品种/性别/生日/年龄、体重、最近 AI 任务、宠物日历记录数、宠友圈小事数、地点点评数和最近关联时间。
- 数据导出新增宠物档案 CSV。

暂未开放：

- 后台直接修正宠物资料。
- 隐藏违规头像、清空 AI 形象图、更换或清空宠友圈封面。
- 合并重复宠物档案。
- 上述动作会影响首页、AI 对话、宠物日历、宠友圈和会话引用，需要 before/after 审计、原因和更细权限后再开放。
...
- 有效举报会自动生成处罚建议和证据快照，运营可在举报中心按建议创建处罚。
...
- 规则接入范围：
  ...
  - 宠友圈评论：命中规则时提示用户修改，避免“已发送但不可见”的体验。
  - 地点点评/新增地点：仍按待审核流程提交，命中规则会抬高风险分并沉淀样本。
- 内容安全页展示最近规则命中样本，为后续模型接入和规则调优预留样本池。
...
- 导出动作会写入 `adminAuditLogs`，action 为 `data.export.download`，记录数据集、文件名、字段和行数。
...
- 业务健康快照覆盖：
  ...
  - 地点审核通过率。
  - 举报有效率。
- 看板明确展示数据口径缺口，避免把尚未采集的移动端行为假装成真实数据。
- 最近 7 天明细表用于核对趋势图背后的日汇总。

当前口径限制：

- 附近发现曝光人数、地图打开次数、POI 搜索次数、地点详情查看次数尚未有移动端事件埋点。
- 严格留存 Cohort 需要每日活跃事件流水；当前只能用 `createdAt` 和 `lastSeenAt` 做轻量近似。
- Push 真实送达/点击依赖厂商通道回执，当前只能展示站内通知与设备 token。
...
4. 通知运营进阶：厂商 Push、发送审批、频控、复杂深链和灰度人群包。
