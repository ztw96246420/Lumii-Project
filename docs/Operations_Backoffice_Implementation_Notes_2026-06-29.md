...
- 更正式的客服质检制度仍待业务确认，例如自然周/月或排班周期 KPI、外包客服结算口径、抽检申诉流程、批量回复双人审批和发送撤回策略。
...
- 配置中心已支持内容安全文本规则：
  ...
  - 复审关键词：命中后进入人工审核。
- 配置中心已支持腾讯云机审开关：
  - `moderation.machineTextEnabled`：启用后，私信聊天、宠友圈小事、评论、地点点评、新增地点、宠物资料文本会调用腾讯云文本内容安全。
...
- 后台修复异常招呼/约遛状态。
- 任意会话全文检索；目前只允许从私信消息举报查看上下文窗口。
- 关系消息页直接隐藏违规会话消息；目前需从举报中心或内容安全任务处理。
...
3. 客服工单进阶：客服质检制度、抽检申诉规则、周/月 KPI 结算口径、外包结算口径、批量回复双人审批和撤回策略。
