...
- 展示状态文件滚动备份、最近备份、备份错误和启动恢复来源；后端保存 state 使用临时文件原子写入，主 JSON 损坏时会尝试从最新有效备份恢复。
...
- AI 回复会保存后台专用的轻量生成快照，包含 provider、生成来源、模型参数、System Prompt hash/摘要、动态上下文 hash、宠物档案快照和 token 用量；移动端 AI 对话接口会剥离该信息，后台查看全文时可用于历史问题追溯。

当前限制：

- 当前只保存轻量生成快照，尚未保存上游模型的完整 request / response 原始报文。
...
- 新增地点审核通过会记录地点贡献者账本：发现新地点 `+10` 贡献分，补充已有地点 `+5` 贡献分；地点目录展示贡献者数量，后台新增贡献者卡片、贡献排行榜和荣誉候选策略，并支持运营手动调整或撤销贡献记录。
...
- App 地点详情的地点本体可被用户举报；举报进入后台举报中心和统一内容安全任务池，`targetType=place`，后台可从举报中心直接修正地点名称、地址、分类、宠物友好状态、支持宠物、标签、电话和营业时间，并自动把举报标记为有效。
- 配置中心新增 `places.publicReviews`，控制公开点评排序、是否只看有图、后端返回上限和地点详情首屏展示条数；移动端地点详情会读取该配置展示对应口径。
...
- 贡献分当前已支持用户本人公开徽章、轻量等级、我的排名、匿名排行榜、荣誉候选提示和后台纠偏；真实活动结算、他人主页贡献身份或兑换规则仍需另行确认。
...
- 扩展回归脚本：`node scripts/smoke-place-contributions.cjs`，覆盖地点贡献账本、手动调整、撤销、`/me.placeContributionSummary`、匿名排行榜、荣誉候选策略、贡献徽章配置发布和 `/app/config` 下发。
- 扩展回归脚本：`node scripts/smoke-place-public-reviews.cjs`，覆盖地点公开点评展示策略、`/app/config.places.publicReviews` 下发、只看有图过滤、举报和运营隐藏。
...
- 通知批次效果统计已接入：移动端通知列表展示上报 `notification.impression`，打开通知上报 `notification.open`，后端按站内通知 `read/readAt`、展示事件和点击事件回算展示率、已读率、打开率、最近展示/点击时间；通知运营页和数据看板同步展示。
...
- Android 厂商 Push/APNs 专项通道、OS 级展示回执、站外审批提醒和值守 SOP。
...
- ECharts 图表覆盖：
  - 用户增长与活跃。
    ...
    - 移动端行为：页面浏览、首页模块曝光、发现曝光、小事卡片曝光、地图打开、POI 搜索、地点详情、通知展示、通知点击。
...
- 已补移动端基础事件埋点：页面浏览、首页模块曝光、AI 形象前端漏斗、发现曝光、附近伙伴加载、小事加载、小事卡片曝光、小事点赞/评论/招呼/约遛点击、地图打开、定位、POI 搜索、地点详情、导航打开、地点收藏、通知展示、通知点击、宠友圈主页查看和反馈进度查看。
- 生产级严格留存 Cohort 仍需要独立事件表、设备去重和长期窗口；当前 JSON state 只适合测试期轻量 DAU、漏斗、Cohort 与行为趋势。
- Push 已支持 Expo ticket/receipt、App 内通知展示/点击和失效 token 标记；OS 级展示回执、Android 厂商 Push/APNs 专项通道仍待生产接入。
...
- 新增 `GET /admin/approvals/pending`。
- 聚合配置发布、系统通知、数据导出、永久封禁、批量处罚、用户业务数据清理和批量客服回复的 `pending_approval` 单据。
- 每条队列项返回来源、标题、说明、申请人、原因、会签进度、剩余审批人数、过期时间、剩余分钟数、严重级别和后台跳转 route。
- 工作台新增“待审批”指标和“待审批值守队列”表格；这里只做分流和跳转，实际审批仍在各业务模块内完成，继续复用现有权限、会签和审计。
- 新增回归脚本：`node scripts/smoke-pending-approval-watch.cjs`，覆盖 7 类审批来源、工作台告警和聚合统计。

当前口径限制：

- ~~这是后台站内值守队列，不等同于企业微信、飞书、邮件或短信等站外通知。~~ 2026-07-10 已接企业微信、飞书、钉钉和通用 Webhook 站外聚合提醒。
- 生产期仍需配置真实 Webhook，并确认审批 owner、值守班表和超时升级 SOP。

## 3.17 站外运营告警与审批提醒

已补生产可配置的站外告警链路：

- `LUMII_ADMIN_ALERT_WEBHOOK_URL` 仅从服务器环境变量读取，后台不回显完整地址。
- 支持自动识别企业微信、飞书、钉钉，也支持通用 JSON Webhook。
- 自动聚合后台 critical/high 运营告警，高风险待审批通过 `high_risk_pending_approvals` 一并提醒。
- 相同告警按内容指纹去重，默认 5 分钟巡检、6 小时持续未恢复后重提醒。
- 供应商 HTTP/业务错误码、超时和恢复结果写入 `adminAlertWebhookDeliveries`。
- 工作台与系统健康页显示配置、最近送达和失败状态；具备配置权限的管理员可手动发送测试告警，动作写审计。
- 上线台账的高风险审批和可观测性缺口会实时读取通道配置与最近发送状态。
- `smoke-observability-alerts.cjs` 已覆盖自动发送、去重、手动测试、失败和恢复。
...
6. 新增地点审核通过/驳回已通知提交人，基础贡献积分账本、贡献身份展示、我的排名、匿名排行榜、荣誉候选策略和后台纠偏已落地。后续是否要展示他人主页贡献身份、真实活动结算或兑换规则待确认。
7. 地点点评审核通过后，App 地点详情已展示公开点评，并支持举报进入后台任务池；公开点评排序、只看有图和首屏条数已由后台配置控制，后续是否需要移动端“查看更多”分页页或用户自选筛选待确认。
8. 维护模式开启时，App 是阻断登录，还是只显示顶部提示？
9. AI 形象任务返还额度的规则已接入：默认仅供应商提交失败、供应商超时和供应商返回失败自动返还；照片不合格、内容安全拦截和运营手动标失败不自动返还，后台人工返还会防重复。
...
1. 处罚策略进阶：多管理员复核、站外审批提醒和处罚建议的命中率复盘；批量处罚审批与后台待审批值守队列已接入基础闭环。
