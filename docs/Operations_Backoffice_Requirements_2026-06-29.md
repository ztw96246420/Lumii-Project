...
- 清除异常 AI 形象图。
- 更换或清空宠友圈封面。
- 合并同一用户下重复宠物档案；跨用户或争议归属合并仍不开放。

### 6.4 AI 灵伴

AI 灵伴后台分为“生成任务”、“媒体素材”、“生成反馈”、“AI 样本池”、“用量成本”和“供应商监控”。

当前实现状态：

- 第一阶段已落地“生成任务”、“媒体素材”、“生成反馈”的运营列表、筛选、处理和 CSV 导出。
- 第一阶段已落地“用量成本”和“供应商监控”的只读运营看板、provider 对比图、Top 错误码和 CSV 导出。
- 已落地“AI 样本池”：支持从任务沉淀提示词优化样本、供应商异常样本和素材质量样本，并支持待复盘、已复核、已忽略状态、筛选、审计和 CSV 导出；移动端生成反馈会自动进入提示词样本，供应商提交/状态异常会自动进入供应商异常样本。
...
- 将任务加入样本集，用于后续提示词评估。
- 标记为供应商异常样本。
- 样本复核或忽略，仅影响后台复盘状态，不直接改变线上 prompt 或移动端展示。
- 自动入池仅产生后台待办，不自动重跑任务、不自动调整 provider、不自动下发配置。
...
- 支持后台新增体重、疫苗/驱虫、备忘。
- 支持软删除、恢复和批量删除/恢复；删除后移动端不再展示，恢复后重新展示，后台保留快照、原因和审计。
- 双人复核和多角色权限暂不开放，后续按生产治理补齐。

所有新增、修正、删除、恢复动作必须：

- 原因必填。
- 写入对应审计，记录 before/after、变更字段、手机号和宠物 ID。
...
- 评论文本上限。
- 聊天消息文本上限。
- 附近发现默认半径，只允许 `3km / 5km / 10km` 三档，首发默认 10km；同时作用于附近伙伴、附近小事和地图 POI。
- 附近小事有效期，当前为 7 天。
- 短信发送冷却和每日上限。
- 审核关键词规则。
- 举报原因枚举。
