...
const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lumii-admin-sanctions-page-'));
...
async function loginUser(phone) {
  await request('/auth/sms/send', {
    body: { deviceId: `admin-sanctions-page-${phone}`, phone },
    method: 'POST',
  });
...
async function main() {
  ...
  const phones = ['19900006911', '19900006912'];
  for (const phone of phones) await loginUser(phone);
  ...
  try {
    ...
    await page.locator('#sanctionBatchPhones').fill(phones.join('\n'));
    await page.locator('#sanctionBatchReason').fill('Visual batch freeze smoke');
    await page.locator('button[data-action="sanction-batch-create"]').click();

    await page.locator('text=Visual batch freeze smoke').waitFor({ timeout: 30_000 });
    await page.locator('button[data-action="sanction-batch-approval-approve"]').waitFor({ timeout: 30_000 });

    console.log('admin sanctions page smoke passed');
  } finally {
